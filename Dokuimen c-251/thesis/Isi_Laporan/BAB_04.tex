...
\section{Model Matematis Sistem Kadar Oksigen Terlarut dalam Air}
\label{sec:Model_Matematis_dan_Input_Kontrol_pada_Sistem_Kadar_Oksigen_Terlarut_dalam_Air}

"Model matematis untuk menggambarkan kadar oksigen terlarut dalam air secara dinamis dapat digambarkan dengan persamaan diferensial yang mencakup dua faktor utama: penambahan oksigen ke dalam air dan konsumsi oksigen oleh organisme. Persamaan dasar yang menggambarkan perubahan kadar oksigen $C(t)$ secara idealadalah:
\begin{align}
    \dfrac{dC(t)}{dt} &
    = K_1 (C_{\text{sat}} - C(t)) - K_2 C(t) + \beta u(t)
    \label{eqn:model_matematis_oksigen}
\end{align}
dimana $C_{\text{sat}}$ adalah kadar oksigen yang diinginkan dalam satuan $mg/L$, $u(t)$ adalah isyarat masukan berupa laju injeksi oksigen dari \textit{aerator} dalam satuan $mg/L/s$, $\beta$ adalah konstanta aktuator, dan $K_1$ serta $K_2$ adalah konstanta yang masing-masing menggambarkan laju penambahan oksigen dan konsumsi oksigen oleh organisme dalam satuan $1/s$ \cite{SmithJ_modeling_2015, jones_aeration_2017}."
